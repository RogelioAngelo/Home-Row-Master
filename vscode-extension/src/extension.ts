...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(
      HomerowWebviewViewProvider.viewType,
      provider,
      {
        webviewOptions: {
...
class HomerowWebviewViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'homerow.webviewView';
    ...
    webviewView.webview.onDidReceiveMessage(message => {
      switch (message.command) {
        ...
        case 'saveStats':
          ...
          break;
        case 'getStats':
          const highScore = this._globalState.get<number>('hrm_highscore', 0);
          const bestStreak = this._globalState.get<number>('hrm_beststreak', 0);
          const bestWpm = this._globalState.get<number>('hrm_bestwpm', 0);
          const mode = this._globalState.get<string>('hrm_mode', 'letters');
          const seqLength = this._globalState.get<number>('hrm_seqlength', 6);
